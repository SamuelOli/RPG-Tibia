...
public class InventoryManager : MonoBehaviour
{
    ...
    public void SetImage(int i, Sprite icon, float color)
    {
        float x = color;
        if (color == 0) { x = .5f; }
        slots[i].transform.GetChild(1).GetComponent<Image>().sprite = icon;
        slots[i].transform.GetChild(1).GetComponent<Image>().color = new Color(color, color, color, x);
    }
    ...
    public void EquipItem(int i)
    {
        if (inventoryItems[i] != null)
        {
            if (inventoryItems[i].heroType == HeroType.Any || inventoryItems[i].heroType == playerStats.hero.heroType)
            {
    ...
    {
        Debug.Log("Drop");
        ItemDropped.DropItem(inventoryItems[i], playerCombat.transform);
